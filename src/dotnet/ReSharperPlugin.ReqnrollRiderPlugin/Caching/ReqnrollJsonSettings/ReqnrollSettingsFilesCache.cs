...
public class ReqnrollSettingsFilesCache(
    Lifetime lifetime,
    IShellLocks shellLocks,
    IPersistentIndexManager persistentIndexManager,
    ReqnrollSettingsProvider settingsProvider,
    ReqnrollSettingsMarshaller marshaller)
    : SimpleICache<ReqnrollSettings>(lifetime, shellLocks, persistentIndexManager, marshaller)
{
    ...
    private void UpdateInProvider(IPsiSourceFile file, ReqnrollSettings? newSettings)
    {
        ...
        var oldSettings = settingsProvider.GetSettings(reqnrollJsonProjectOwner);
        if (!settingsProvider.TryUpdate(reqnrollJsonProjectOwner, GetConfigSource(file), newSettings))
            return;
